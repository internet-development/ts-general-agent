...
| `peer-awareness.ts` | Dynamic peer SOUL discovery from plans, workspaces, threads. `registerPeerByBlueskyHandle()` creates peer records from Bluesky workspace URL mentions. `linkPeerIdentities()` merges Bluesky handle ↔ GitHub username into single peer record (Scenario 35). `getPeerGithubUsername()` resolves Bluesky handles to GitHub usernames for PR reviewer assignment. `getPeerUsernames()` returns all known peer GitHub usernames for round-robin prevention | Config, Memory |
...
|--------|---------|--------------|
| `engagement.ts` | Relationship tracking, notification prioritization, `isLowValueClosing()` (verbose closing/acknowledgment detection — Scenario 15), `shouldRespondTo()` (notification filtering with hard-block for closings), `analyzeConversation()` (3-comment saturation cap + round-robin prevention — Scenario 13, 25) | Used by scheduler, multiple local-tools depend on it |
